...
def list_accounts(token, id):
    id = id[
        0
    ]  # this ISN"T R - A TOUPLE OF 1 ELEMENT IS STILL A TOUPLE, NOT A SINGLE ELEMENT
    res = requests.get(
        f"https://ob.nordigen.com/api/v2/requisitions/{id}/",
        headers={"accept": "application/json", "Authorization": "Bearer " + token},
    )
    print(res.json())
    return res.json()
...
def get_transasctions(token, id, date):
  ...
  """
    res = requests.get(
        f"https://ob.nordigen.com/api/v2/accounts/premium/{id}/transactions",
        headers={"accept": "application/json", "Authorization": "Bearer " + token},
    )
    return res.json()
...
tips = {
    ...
    "Food and Drink": [
        ...
        "Think",
        "sometimes swapping meat for plants can make your favourite dish cheaper",
    ],
    ...
    "Loan": [
        "Different loans have different costs - prioritise repaying those with the highest interest",
        "Struggling to repay your loan? - Think about discussing a payment plan with your debitors",
    ],
    "Savings and Investments": [
        "Saving for your future - just don't forget about the present too"
    ],
    "Services": ["Have you checked your statements - maybe you", "", ""],
    "Shopping": ["", "Make sure you shop around - prices for the same item may vary",],
    "Tax": [
        "You can often deduct some expenses to reduce your tax base",
        "",
        "Have you checked your tax returns - make sure you are paying the right amount of tax",
        "a",
    ],
    "Transport": [
        "Season Tickets can be cheaper than single ticket - if you travel a lot"
    ],
    "Travel": [
        "Travelling a lot? - Make sure you shop around for the best deals",
        "",
        "Make sure you plan a budget for your holidays - spending can accumulate fast",
    ],
    "Utilities": [
        "Have had the same provider for a while - make sure you shop around for the cheapest provider",
        "keep an eye on your bills - you may be paying off estimates",
        "Have you considered changing your lightbulb?",
    ],
}
