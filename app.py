...
if not st.session_state.name:
    with st.form(key="login", clear_on_submit=False):
        ...
        logging_in = st.form_submit_button(
            label="login"
        )  # , on_click=login, args=(conn, user, password),   )
        register_sub = st.form_submit_button(
            label="register"
        )  # , on_click=register, args=(conn, user, password))
...
if st.session_state.name:
    ...
    st.session_state.fernet = generate_fernet(
        st.session_state.salt, st.session_state.token
    )
    ...
    with st.expander("add a new provider:"):
        ...
        st.button(
            ...
            args=(
                ...
                providers[
                    (providers.countries == country) & (providers.name == bank)
                ].id.to_list()[0],
                conn,
        ...
        st.button(
            "connect the sandbox bank (RECOMMENDED)",
            on_click=build_link,
            args=(
                st.session_state.token,
  ...
  """
    if st.session_state.obj:
        ...
        st.session_state.obj = st.button(
            "Load your budget", on_click=load_budget, args=(conn, st.session_state.name)
        )
    else:
        st.session_state.obj = st.button("Set a Budget", key=None)

    if st.session_state.obj:
        st.session_state.income = st.session_state.avail = st.number_input(
            "what is your monthly income?", 0, step=1
        )
        ...
        for category in categories:
            if category == "Other":
                amount = st.number_input(
                    f"{category}",
                    min_value=0,
                    value=st.session_state.avail,
                    max_value=st.session_state.avail,
                    step=1,
                    key=None,
                )
        ...
        st.button(
            ...
            args=(st.session_state.budget, conn, st.session_state.name, categories),
        )
        st.button("reset budget")
        st.button("reload budget")
    # Display budget to user
    budget = pd.DataFrame.from_dict(
        st.session_state.budget, orient="index", columns=["Amount"]
    )
    ...
    if not budget.empty:
        budget
        plot = budget.plot.pie(
            y="Amount",
            figsize=(5, 10),
            subplots=False,
            legend=False,
            autopct=lambda p: func(p, budget.iloc[:].values),
        ).figure
    ...
    def wrap_transactions(ids):
        with st.spinner("Loading transactions, Please wait"):
            for id in ids:
                st.session_state.transactions.update(
                    {
                        id: get_transasctions(
                            st.session_state.token,
                            id,
    ...
    if st.session_state.transactions:
        ...
        for account in st.session_state.transactions:
            frame = pd.json_normalize(
                st.session_state.transactions[account]["transactions"]["booked"]
            )
            frame.rename(
                columns={
        ...
        for data in transactions:
            st.dataframe(
                data[["Type", "Date", "Amount", "Category", "Destination", "Info"]],
                width=4000,
            )
        ...
        with st.spinner("Loading transactions - This might take some time"):
            for account in accounts.accounts:
                ...
                transactions = pd.json_normalize(
                    raw_transactions["transactions"]["booked"]
                )
