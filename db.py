  ...
  """
    ...
    if not results or (dict(results) != categories):
        ...
        for label, id in categories.items():
            c.execute("INSERT INTO categories(category,id) VALUES(?,?);", (label, id))
        db.commit()
    return True
...
def register(db, username, password):
    ...
    if query:
        ...
        try:
            c.execute(
                "INSERT INTO users (enduser_id, reference, username, password, salt) VALUES(?,?,?,?,?)",
                (
                    uuid4().bytes_le,
                    uuid4().bytes_le,
                    username,
...
def load_budget(db, username):
  ...
  """
    c = db.cursor()
    query = c.execute(
        """SELECT category, amount FROM budget JOIN categories 
...
def save_budget(budget, db, username, categories):
  ...
  """
    ...
    for label, amount in budget.items():
        ...
        query = c.execute(
            "SELECT * FROM budget WHERE users_id = ? AND categories_id = ?",
            (id, categories[label]),
        ).fetchall()
        if query:
            #  if so, update
            c.execute(
                "UPDATE budget SET amount = ? WHERE users_id = ? AND categories_id = ?",
                (amount, id, categories[label]),
            )
        # if not insert
        else:
            c.execute(
                "INSERT INTO budget(users_id, categories_id, amount) VALUES(?,?,?)",
